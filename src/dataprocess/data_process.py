...
class DataProcess:
        ...
        for key, value in input_dict.items():
            if type(value) is dict:
                for key_in_dict, value_in_dict in value.items():
                    ...
                    final_list.append(list(value_in_dict))
            else:
    ...
    @staticmethod
    def make_torch_dataset(input_dataset_directory: str, output_csv_file: str, sub_folder: bool):
        """
        Read data set under the given directory, return data and label
        :param  input_dataset_directory: input data directory
        :param  output_csv_file: output csv file (filename, label)
        :param  sub_folder: Set True if dataset folder contains sub folders
        """
        # Get sub-folder names
        if sub_folder is True:
            sub_folders = FileUtil.get_folder_names(input_dataset_directory)
            ...
            for sub_folder in sub_folders:
                ...
                filenames = FileUtil.get_file_names(os.path.join(input_dataset_directory, sub_folder))
                filenames = [os.path.join(sub_folder, file_name) for file_name in filenames]
                filenames_list.extend(filenames)

                # Make label
                label_list.extend([label_num]*len(filenames))
                label_num += 1
        else:
            print("Currently not supported")

        # Store lists into dataframe
        dataframe = pd.DataFrame(list(zip(filenames_list, label_list)), columns=['filename', 'label'])

        # Write to csv
        FileUtil.dataframe2csv(dataframe, output_csv_file, header=True)
